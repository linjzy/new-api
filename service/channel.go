...
func disableChannelKey(channelError types.ChannelError, usingKeyIndex int, reason string) bool {
	...
	if success {
		...
		notifyType := formatNotifyType(channelError.ChannelId, common.ChannelStatusAutoDisabled)
		if usingKeyIndex >= 0 {
			// Persist the retired key before retrying, but do not wait for an
			// email or webhook to finish before selecting the next credential.
			gopool.Go(func() { NotifyRootUser(notifyType, subject, content) })
		} else {
