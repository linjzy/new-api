...
- 仅检测启用且配置了 `gpt-6-astra` 的渠道，手动或自动停用均跳过，以每轮开始时间为基准每 5 分钟一轮（调度轮询误差最多 15 秒），最多并行 3 个渠道，每个渠道最长 2 分钟。独立系统任务使用现有数据库租约避免多节点重复执行；上一轮未完成时不会重叠启动。
...
- 详情支持删除此次结果或清空当前渠道全部检测结果，均需确认并由服务端校验渠道写权限。删除旧记录保留当前判定；删除最新结果或清空后等待新检测，不将更早的通过记录恢复为路由许可。历史写入和删除使用事务，避免完成中的检测重新带回已经删除的历史；后续检测可以正常生成新记录。
- 结果只写入独立 `astra_iq_results` 表，现有渠道/密钥/计费表结构不变；历史最多保留 24 轮，详情列在 MySQL 使用 LONGTEXT，SQLite/PostgreSQL 使用 TEXT；迁移保留原有历史。切换客户端后旧审批失效，须由官方客户端复检通过。每个启用密钥每天约 288 次上游检测请求，会产生上游用量，沿用现有渠道测试日志记录方式。
