...
- 渠道标题旁“检测设置”提供开始时间、结束时间、间隔（分钟）、失败停用，适用于所有 Astra 渠道。每天按北京时间 UTC+8 执行，支持跨午夜；相同时间表示全天，间隔支持 1～1440 分钟。默认 00:00→00:00、5 分钟、失败停用开启。设置保存为共享数据库单个 JSON 值，原子更新、重启保留；GET 使用渠道读权限，PUT 使用渠道写权限。
...
- 结果只写入独立 `astra_iq_results` 表，现有渠道/密钥/计费表结构不变；历史最多保留 24 轮，详情列在 MySQL 使用 LONGTEXT，SQLite/PostgreSQL 使用 TEXT；迁移保留原有历史。切换客户端后视为暂无适用结果，继续由官方客户端生成新的检测记录。默认全天 5 分钟间隔时，每个启用密钥每天约 288 次上游检测请求，会产生上游用量，沿用现有渠道测试日志记录方式。
