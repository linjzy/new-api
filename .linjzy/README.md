...
- 无当前检测结果允许调用，包括首次未测、删除最新结果、题目/客户端版本或凭据/上游配置变化。仅在“失败停用”开启且最新适用结果为失败或异常时跳过渠道，复检正确后恢复；关闭“失败停用”仍继续检测、保存结果。时段外保留最近判定，不再因固定 10 分钟过期阻断调用；读取设置或结果失败仍拒绝选用，避免把数据库故障误当成没有结果。
...
- 卡片底部在分组名称后同一行显示最近 24 轮的精细色块，保持原卡片尺寸：绿=通过、红=答错、橙=异常，未测留白，色块上下无额外文案。点击或键盘激活任意历史色块，可查看当轮题目、答案、模型回复、耗时、首字延迟、Token 用量、HTTP 状态、接口及客户端版本；旧记录未保存的指标留空。前端每 30 秒刷新，路由仍依据最新适用结果和检测设置。
- 状态条最多展示最近 24 格，以实际任务轮次定位，结合整轮耗时和调度轮询容差消除正常连续检测的假空档；真正缺测、暂停和删除造成的空档仍留白。旧记录以完成时间减耗时回推开始时间；任务历史清理后仍能显示保留的检测结果，历史最多保存 24 轮。
