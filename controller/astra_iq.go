...
func GetAstraIQStatus(c *gin.Context) {
	...
	runs, err := model.GetAstraIQRuns()
	if err != nil {
		common.ApiError(c, err)
		return
	}
	common.ApiSuccess(c, gin.H{"enabled": true, "interval_minutes": settings.IntervalMinutes, "server_time": common.GetTimestamp(), "runs": runs, "results": views})
}
