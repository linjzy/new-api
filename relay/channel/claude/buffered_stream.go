...
func NewBufferedStreamAccumulator() *BufferedStreamAccumulator {
	return &BufferedStreamAccumulator{
		response: dto.ClaudeResponse{
			Type: "message",
			Role: "assistant",
		},
		...
		thinkBuffers: make(map[int]*strings.Builder),
	}
}
...
func (a *BufferedStreamAccumulator) block(index int) *dto.ClaudeMediaMessage {
...
func (a *BufferedStreamAccumulator) finalizeToolInput(index int) error {
...
func (a *BufferedStreamAccumulator) Process(event *dto.ClaudeResponse) error {
	if event == nil {
		return nil
	}
	...
	case "content_block_delta":
		...
		case "text_delta":
			...
			if event.Delta.Text != nil {
				builder, ok := a.textBuffers[index]
				if !ok {
					builder = &strings.Builder{}
					builder.WriteString(block.GetText())
					a.textBuffers[index] = builder
				}
		...
		case "thinking_delta":
			...
			if event.Delta.Thinking != nil {
				builder, ok := a.thinkBuffers[index]
				if !ok {
					builder = &strings.Builder{}
					if block.Thinking != nil {
						builder.WriteString(*block.Thinking)
					}
					a.thinkBuffers[index] = builder
				}
				builder.WriteString(*event.Delta.Thinking)
				thinking := builder.String()
				block.Thinking = &thinking
			}
		case "signature_delta":
			block.Signature += event.Delta.Signature
		case "citations_delta":
			if len(event.Delta.Citation) > 0 {
				citation := append([]byte(nil), event.Delta.Citation...)
...
func (a *BufferedStreamAccumulator) FinalResponse(model string) (*BufferedResponse, error) {
	if !a.started {
...
func ClaudeBufferedStreamHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*dto.Usage, *types.NewAPIError) {
	...
	accumulator := NewBufferedStreamAccumulator()
	...
	scanContext, cancelScan := context.WithCancel(c.Request.Context())
	...
	go func() {
		...
		select {
		case scanEvents <- scanEvent{err: scanner.Err(), done: true}:
		case <-scanContext.Done():
		}
	}()
	defer func() {
	...
	if streamingTimeout <= 0 {
		streamingTimeout = time.Nanosecond
	}
	timeout := time.NewTimer(streamingTimeout)
	defer timeout.Stop()
	streamStopped := false
	for !streamStopped {
		var scanned scanEvent
		select {
		case <-c.Request.Context().Done():
			return nil, types.NewError(c.Request.Context().Err(), types.ErrorCodeBadResponse, types.ErrOptionWithSkipRetry())
		case <-timeout.C:
			return nil, types.NewError(
				fmt.Errorf("Claude stream timed out after %s", streamingTimeout),
				types.ErrorCodeBadResponse,
			)
		case scanned = <-scanEvents:
		}

		if scanned.done {
			if scanned.err != nil {
				return nil, types.NewError(scanned.err, types.ErrorCodeBadResponse)
			}
			break
		}
		if !timeout.Stop() {
	...
	finalUsage := buildMessageDeltaPatchUsage(response.ClaudeResponse, claudeInfo)
	if finalUsage.OutputTokens == 0 {
		finalUsage.OutputTokens = claudeInfo.Usage.CompletionTokens
	}
	if finalUsage.BillingUsage == nil {
		finalUsage.BillingUsage = dto.CloneBillingUsage(claudeInfo.Usage.BillingUsage)
	}
