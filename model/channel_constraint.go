...
// On false, it returns the kind of the first violated filter (excluded channels,
// request_path, task_plugin_identity, then responses_websocket) for error
// attribution.
func ChannelSatisfiesFilters(ch *Channel, modelName string, filters []dto.ChannelFilter) (bool, dto.ChannelFilterKind) {
	if ch == nil {
		return false, ""
	}
	if !AstraIQAllowsChannel(ch, modelName) {
		return false, "astra_iq"
	}
...
func filterCandidateIDs(ids []int, modelName string, filters []dto.ChannelFilter) (kept []int, emptiedBy dto.ChannelFilterKind) {
	...
	kept = filterAstraIQCandidateIDs(ids, modelName)
	if len(kept) == 0 {
		return kept, "astra_iq"
	}
	for _, kind := range filterEvalOrder {
...
func channelMatchesFilter(ch *Channel, modelName string, filter dto.ChannelFilter) bool {
	switch filter.Kind {
	case dto.FilterExcludeChannels:
