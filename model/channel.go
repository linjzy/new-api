...
func EnableAutoDisabledChannelKey(channelId int, keyIndex int, expectedKey string, expectedDisabledTime int64) bool {
	...
	statusChanged := false
	defer func() {
		// Run after the per-channel lock is released. A credential-only update
		// does not change pricing or task routing availability.
		if recovered != nil && statusChanged {
	...
	cacheRecoveredChannelKey(recovered)
	return true
}
